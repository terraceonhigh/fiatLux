...
**AO3 chapter numbers are not guaranteed to be plan chapter numbers, and any gap is deliberate.** As of 2026-09-10 they agree through slot 12: posting *Isolation* into AO3 slot 7 on 2026-08-21 closed the offset that had *Boil Water* at AO3 7 for plan 8 and *Ground* at AO3 9 for plan 10, shifting each up one, and everything since has landed at its plan number. One unposted draft still holds slot 6 for *Notifiable*, and because a draft holds a real position the numbering stays aligned while it sits there. **Do not assume alignment** — *Off-Label, Part II* at plan 6 would reopen the offset for every chapter above it. The mechanism, learned the hard way on 2026-08-20:
...
- **A placeholder draft is untitled, with one `<p>` of lorem ipsum, saved with `Save Draft`.** That is the established convention — slot 6 is built that way, and slot 8 was until *Isolation* went up and *Boil Water* moved into it. Titling one would put the chapter's name in the owner's chapter list before it exists.
